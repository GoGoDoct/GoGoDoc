...
| 평가기 | 데이터셋 | 목적 |
|--------|----------|------|
| `chat_eval.py` | `chat_scope_golden.jsonl` 104건 | 질문 분류, 위험질문 차단율, 응급 라우팅 |
| `chat_answer_eval.py` | `chat_answer_golden.jsonl` 10건 | 허용 질문의 RAG 답변 근거성, 출처, 환각 방지 |
| `chat_answer_service_eval.py` | `chat_answer_service_golden.jsonl` 28건 | 최신 결과 연결, `question_type`·`route_reason`, answer LLM 호출 정책 |
