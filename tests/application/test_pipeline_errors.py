...
def test_empty_text_raises_parse_error():
    # 텍스트 추출 실패 - 치명 오류
    pipeline = Pipeline(parser=_Parser(""), llm=_LLM())
...
def test_bad_json_raises_parse_error():
    ...
    pipeline = Pipeline(parser=_Parser(), llm=_LLM(parse_out="JSON 아님"))
    with pytest.raises(ParseError):
        pipeline.run("dummy.pdf", _profile())
...
def test_invalid_row_skipped():
    ...
    pipeline = Pipeline(parser=_Parser(), llm=_LLM(parse_out=parse_out))
...
def test_interpret_failure_is_resilient():
    ...
    pipeline = Pipeline(parser=_Parser(), llm=_LLM(interpret_raises=True))
    report = pipeline.run("dummy.pdf", _profile())
