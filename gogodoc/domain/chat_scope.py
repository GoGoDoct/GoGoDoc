...
_RULE_PATTERNS: list[tuple[QuestionType, str, re.Pattern[str]]] = [
    ...
    (
        ...
        re.compile(
            ...
            r".*(먹|복용|시작|끊|바꾸|추천|계속|같이|병용|중단|고려)|"
            ...
            r"((?<![가-힣])약(?!간)|주사|치료제|스타틴|메트포민|혈압약|당뇨약|고지혈증약|"
            r"철분제|인슐린|통풍약|약물치료)"
            r".{0,16}(필요|해야|써야|먹어야|맞아야|되나|될까|될까요|고려)|"
    ...
    (
        ...
        re.compile(
            ...
            r"(이에요|이예요|인가요|입니까|맞나요|인지|일까요|아니야|아닌가요|아니죠|이라는|진단)|"
            r"(당뇨|당뇨병|고혈압|고지혈증|암|종양|간경화|신부전)\s*"
            r"(이야|야|인가|일까|일까요|아니야|아닌가요|아니죠|일\s*가능성|가능성)|"
    ...
    (
        ...
        "unsupported_rule",
        re.compile(
            r"작년|저번|지난\s*번|지난\s*검사|이전\s*검사|과거\s*검사|전\s*검사|"
            r"이전\s*검진|과거\s*검진|전\s*검진|추세|"
            r"(작년|저번|지난|이전|과거|전\s*(검사|검진)).*(비교|대비|보다|오른|올랐|높아졌|낮아졌)|"
            r"근처\s*병원|병원\s*추천|"
            r"비용|가격|보험|실비|싼\s*병원|저렴한\s*병원|"
            r"(스타틴|메트포민|위고비|마운자로|졸피뎀|알닥톤|타이레놀|"
            r"(?<![가-힣])약(?!간)|철분제|인슐린|통풍약|치료제|약물치료).{0,16}"
            r"(뭔|뭐야|무엇|설명|효과|부작용)|"
    ...
    (
        ...
        re.compile(
            ...
            r"(받아야|해야|필요|찍어야|찍는\s*게\s*좋|할까요|하나요|하면\s*좋|해도\s*(되|돼)|고려)|"
            r"(정밀\s*검사|추가\s*검사|조직검사|추적\s*검사).{0,16}"
            r"(받아야|해야|필요|할까요|하나요|하면\s*좋)|"
            r"(간염|암|종양|지방간|간경화|당뇨|당뇨병|고혈압|고지혈증|신부전|갑상선기능저하증|"
            r"갑상선기능항진증).{0,6}검사.{0,12}(받아야|해야|필요|할까요|하나요)|"
    ...
    (
        ...
        "summary_rule",
        re.compile(
            r"검진\s*결과.*(확인해야|신경\s*쓸|요약|전체|전반|제일\s*(문제|신경))|"
            r"(내|제)?\s*(검진\s*)?결과.*(쉽게|풀어|요약|전체|전반)|"
            r"(내|제)?\s*(검진\s*)?결과.*(가족|친구|보호자)(에게|한테|도)?\s*.{0,12}"
            r"(설명|말해|풀어|전달)"
        ),
    ),
    (
...
_SHORTHAND_DIAGNOSIS = re.compile(
    ...
    r"(?:이|가|은|는)?\s*"
    r"(?:이야|야|이지|인가요|인가|일까|일까요|인지|맞나요|맞지|맞죠|맞는지|맞는\s*거지|맞는거지|"
    r"아니야|아닌가요|아니죠|확인\s*(해줘|해|해주세요|해봐|해줘요)?|봐줘)?\s*\??$"
)
_DIAGNOSIS_CONTEXT = re.compile(
    rf"{_SHORTHAND_DIAGNOSIS_TARGET}.{{0,12}}"
    r"(의심|소견|가능성|일\s*수|으로\s*봐야|걸린|걸렸|맞아|맞나|맞나요|맞는지|맞지|맞죠|"
    r"진단|확정)|"
    rf"{_SHORTHAND_DIAGNOSIS_TARGET}\s*(?:인지|맞나|맞는지|아닌지|일지).{{0,12}}(봐|확인|알려)|"
...
_CHECKUP_CATEGORY_TERMS = (
    "콜레스테롤", "혈당", "혈압", "간수치", "간기능", "빈혈",
    "신장", "신장기능", "신장 기능", "콩팥", "콩팥기능", "콩팥 기능",
    "통풍", "비만", "체중", "전해질", "공복혈당장애",
)
