...
_RULE_PATTERNS: list[tuple[QuestionType, str, re.Pattern[str]]] = [
    ...
    (
        ...
        re.compile(
            r"용량|몇\s*(알|정)|증량|감량|"
            r"(약(?!간)|스타틴|메트포민|혈압약|고지혈증약|당뇨약|치료제).{0,20}몇\s*(mg|밀리그램)|"
    ...
    (
        ...
        re.compile(
            ...
            r"(높|낮|이상|초과|미만|수치|검사|검진|결과|이면|라면|나오).{0,28}약(?!간)\s*\??\s*$|"
            r"(스타틴|메트포민|혈압약|고지혈증약|당뇨약|아스피린|위고비|마운자로|졸피뎀|알닥톤|"
            r"타이레놀|한약|철분제|인슐린|통풍약|치료제|약물치료)"
            r"\s*(이|가|은|는|을|를)?\s*\??\s*$|"
            r"(낮추|높이|조절|관리).{0,20}(약(?!간)|주사|치료제).{0,12}(뭐|무엇|알려|추천|필요)|"
            r"(약(?!간)|주사|치료제|철분제|인슐린|통풍약)\s*(이|가|은|는|을|를)?\s*(뭐|무엇|알려|추천)|"
            r"(약(?!간)|주사|치료제|혈압약|당뇨약|고지혈증약|철분제|인슐린|통풍약|약물치료)"
