...
- 규칙 기반 명확 허용: 검진 항목명과 검진 해석 의도가 함께 있는 질문은 LLM fallback 전에 허용
...
평가와 스모크 검증은 UI와 분리된 백엔드 경로에서도 기능이 안전하게 작동하는지 확인한다.

- 스코프 분류 평가: 위험 질문 차단율과 과차단율 측정
- 답변 평가: 근거 적중, 출처 인용, 수치 환각, 진단어 등장 여부 측정
- 통합 정책 평가: `ChatAnswerService` 전체 경로에서 라우팅, 질문 유형, 답변 LLM 호출 수, 근거 상태 측정
...
| `unsupported` | 과거 추세 비교, 실제 병원 추천, 비용·보험 등 현재 미지원 질문 | 현재 지원 범위 안내 |
...
- rule이 검진 항목명과 해석 의도를 명확히 확인한 질문은 `checkup_item_rule`로 허용해 LLM 분류기 과차단을 줄인다.
- LLM fallback 실패 시 보수적으로 차단한다.
- LLM fallback에는 최신 검진 결과나 전체 DB를 넣지 않는다.

`checkup_item_rule`은 항목명 단서만으로 허용하지 않는다. `수치`, `검진`, `의미`, `어때`, `봐줘`, `정상범위`, `관리`처럼 검진 결과 해석 의도가 함께 있어야 한다. 증상·진단·처방·복약·용량·응급 rule은 항상 이 허용 rule보다 먼저 평가된다.
...
4. 식별된 항목이 공인 reference에는 있지만 최신 검진 결과에 없으면 개인 수치 판정이 아니라 일반 정보임을 명시한다.
5. 질문에서 관련 항목·카테고리를 찾지 못했지만 허용 질문이라면 최신 검진 결과의 주의·이상 항목을 fallback 근거로 사용할 수 있다.
6. fallback으로도 사용할 근거가 없으면 근거 부족 안내를 반환한다.
7. `주의`, `이상`, `응급`, `tracking_items` 기반의 전체 요약형 답변은 결정적 요약 경로에서 다룬다.

### 항목명 매칭 정책

사용자가 항목명을 띄어 쓰거나 일부 오타로 입력해도 최신 결과 항목에 안전하게 연결하되, 불확실하면 답변 생성 LLM으로 넘기지 않는다.

- compact exact: 공백·하이픈·슬래시·괄호·마침표 차이와 한국어 조사를 제거해 공식명·동의어를 비교한다.
- report-gated alias: `감마`, `당화`, `총콜`, `허리`, `갑상선` 같은 짧은 표현은 대상 항목이 최신 결과에 있을 때만 개인 결과 근거로 사용한다.
- 제한 fuzzy: 전체 질문 fuzzy는 금지하고, 최신 결과에 있는 공식명·동의어 후보에 대해서만 보수적 임계값으로 비교한다.
- 카테고리 우선 처리: `혈당 관리`, `복부비만 생활습관`처럼 포괄 카테고리로 이미 해석된 표현은 항목명 오타 후보로 다시 평가하지 않는다.
- 공식 항목명이 최신 결과에는 없지만 reference에 있으면 `reference_only_answer`로 일반 정보를 제공한다.
- 항목명처럼 보이나 안전하게 특정할 수 없으면 `item_match_uncertain`으로 안내하고 answer LLM 호출은 0회로 유지한다.

### 근거 상태 구분

| 상태 | 설명 | `route_reason` |
|------|------|----------------|
| 개인 결과 근거 | 질문 항목이 최신 검진 결과에 있고 수치·상태를 함께 사용 | `rag_answer` |
| 일반 reference 근거 | 질문 항목 또는 카테고리가 공인 근거에는 있지만 최신 검진 결과에는 없음 | `reference_only_answer` |
| 보고서 fallback | 질문에서 특정 항목을 찾지 못했지만 최신 결과의 주의·이상 항목으로 답변 | `report_fallback_answer` |
| 항목명 불확실 | 짧은 alias·오타·혼합 질문이 안전하게 하나의 항목으로 확정되지 않음 | `item_match_uncertain` |
| 근거 부족 | reference도 보고서 fallback도 사용할 수 없음 | `no_grounding` |

사용자 답변은 개인 결과 근거와 일반 reference 근거를 혼동하지 않도록 본문에서 구분한다.
...
`ChatAnswerService` 통합 평가는 분류와 답변 품질 사이의 오케스트레이션 정책을 검증한다.

- 차단·응급·자해·범위 밖 질문의 answer LLM 호출 수: 0
- `checkup_summary` 질문의 answer LLM 호출 수: 0
- 허용 grounding 질문의 answer LLM 호출 수: 1
- `question_type`, `route_reason`, `context_item_names`, `sources` 기대값 일치
- 개인 결과 근거, 일반 reference 근거, 보고서 fallback, 근거 부족 상태 구분
...
```json
{
  ...
  "expected_type": "checkup_explanation",
...
스모크 CLI는 Streamlit 화면과 무관하게 최신 DB row와 답변 서비스 연결을 확인한다.
...
3. 백엔드 성공 경로는 CLI 스모크 검증으로 독립 확인한다.
4. UI는 `ChatUiContract`를 통해 연결하되, 화면 개편과 충돌하지 않도록 담당 범위를 분리한다.
