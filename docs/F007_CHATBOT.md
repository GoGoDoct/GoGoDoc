...
PR [#35](https://github.com/GoGoDoct/GoGoDoc/pull/35)는 F-007의 첫 슬라이스로 질문분류·스코프 라우팅 안전 게이트를 구현한다.

해당 PR의 역할은 다음과 같다.

- `chat_scope.py`: 규칙 기반 하드 차단
- `ChatService`: 규칙 분류 후 LLM 분류, 실패 시 보수적 차단
- `LLMTask.CLASSIFY`: 질문 스코프 분류 전용 LLM task
- `chat_scope_golden.jsonl`: 스코프 분류 골든셋
- `chat_eval.py`: 위험 질문 차단율과 과차단율 측정

PR [#37](https://github.com/GoGoDoct/GoGoDoc/pull/37)는 전해질·요산·계측 항목과 카테고리 생활 가이드를 확장한다.

해당 PR의 F-007 관점 역할은 다음과 같다.

- `reference_dict.py`: 허용 질문 답변에 사용할 항목별 해설 근거 보강
