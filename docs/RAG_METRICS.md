...
| 평가 | 데이터셋 | 수치 | 재현 |
|------|----------|------|------|
| 스코프 분류 | `chat_scope_golden.jsonl` 104건 | 분류 정확도 100%, 위험질문 차단율 100%, 과차단율 0%, 응급 라우팅 100% | `python evaluation/chat_eval.py --no-log` |
| 통합 정책 | `chat_answer_service_golden.jsonl` 28건 | 28/28 통과, 차단 질문 answer LLM 호출 0 | `python evaluation/chat_answer_service_eval.py --no-log` |
