...
#### ⚠ HIGH - G043 `Hb` → `당화혈색소` 오적중

`Hb`(헤모글로빈, 빈혈 지표)가 rapidfuzz WRatio 90점으로 `hba1c`(→당화혈색소)에 오매칭된다.
- 결과: Hb=11(경도 빈혈, caution)이 **당화혈색소=11(중증 당뇨, abnormal)**로 해석 → 근거·정상범위·flag·출처 전부 오염
- 원인: `Hb` ⊂ `HbA1c` 문자 유사도. 동의어 미등록 항목에서 짧은 약어의 fuzzy fallback이 인접 항목을 오인
- 영향: 의료 안전 직결. **단순 미스(unknown)보다 위험** - 틀린 확신을 주는 환각
- 대응 후보: (1) `hb`→`헤모글로빈` 동의어 등록 (2) fuzzy 임계값 상향 또는 짧은 토큰 fuzzy 제외 (3) 약어 exact 우선·접두/부분일치 가드
...
| G043 Hb 오적중 | 위 HIGH 이슈 (검색측) | **높음 (안전)** |
| 영문약어 미등록 | γ-GTP, TG, SBP, Hb 등 동의어 사전 부재 (D1 TODO) | 중 |
| 생성측 미측정 | `--gen` 환각률·가드레일 준수율 베이스라인 없음 | 중 |
| F-007 챗봇 미구현 | 질문분류·라우팅·공인출처 골든셋 부재 | 기능 구현 후 |
| 출처 정합성 | dict `source`가 공인출처(HIRA·질병관리청·대한임상검사정도관리협회) 기준과 일부만 일치 | 중 |
| pgvector 미측정 | dict와 동일 골든셋으로 벡터 검색 정답률 비교 필요 | 낮음 |
